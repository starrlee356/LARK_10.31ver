...
def process_logical_queries(logical_query, qtype, idx):
    ...
    question_file_path = os.path.join(f"{args.output_path}","LARK_step_questions",f"{qtype}_{idx}_question.json")
...
def main():
    ...
    step_question_path = os.path.join(f"{args.output_path}","LARK_step_questions")
    ...
    for qtype, qpattern in logic_processor.query_structs.items():
        ...
        arg = zip(logical_queries,
                   [qtype]*llq, range(llq))
        with mp.Pool(mp.cpu_count()) as pool:
            pool.starmap(process_logical_queries, tqdm(arg, total=llq))
